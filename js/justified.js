...
(function () {
  ...
  function run() {
    ...
    document.querySelectorAll('.inline-gallery:not([class*="inline-gallery--"])').forEach(function (g, idx) {
      ...
      /* Gallery 3 (first default gallery): custom visual order on mobile.
         orders[i]      = CSS order value for the image at DOM index i.
         visualOrder    = DOM indices listed in the desired visual sequence,
                          so processGallery pairs the right images per row. */
      ...
      /* Gallery 6 (second default gallery): custom visual order on mobile. */
    ...
    /* split-2 galleries */
    document.querySelectorAll('.inline-gallery--split-2').forEach(processGallery);

    /* lookbook-v2 galleries — CSS flattens the nested wrappers via display:contents.
       Galleries 1 and 2 (idx 0 and 1): first photo gets full width; rest are paired.
       Gallery 7 (idx 2+): all photos paired normally. */
  ...
  document.addEventListener('posts:loaded', function () {
    var imgs  = Array.from(document.querySelectorAll(
      '.inline-gallery--split-2 .gallery-img, ' +
      '.inline-gallery:not([class*="inline-gallery--"]) .gallery-img, ' +
      '.inline-gallery--lookbook-v2 .gallery-img'
    ));
    var total = imgs.length;
    var done  = 0;
    if (!total) return;

    function tick() { if (++done >= total) run(); }
